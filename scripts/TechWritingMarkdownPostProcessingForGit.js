  ...
  const sections = [
    {
      ...
      content:
        "post processing script, that turns a custom inline footnote string with the format ((fn: )) (where ( are curley braces)) into a numbered footnote list, that is also compatible with git. It also parses the mermaid diagrams inside the document to pictures"
    },
    {
      header: "Options",
      optionList: [
        ...
        },
        {
          ...
          alias: "n",
          description:
            "Skips the diagram generating for performance optimization. Only existing ones will be used. Other links will point to non existing resources."
        }
      ]
    }
  ];
...
function checkOptions() {
  ...
  if (Object.keys(options).length === 0) {
    logger.error(
      "No input file and output file specified. Use -h or --help for further information"
    );
    process.exit(4);
  }
  ...
  if (!options["input-file"]) {
    logger.error(
      "No input file specified. Use -h or --help for further information"
    );
    process.exit(4);
  }
  ...
  if (!options["output-file"]) {
    logger.error(
      "No output file specified. Use -h or --help for further information"
    );
    process.exit(4);
  }
...
function processMermaid() {
  ...
  var res = str.replace(regex, function(x) {
    ...
    if (imageNameArray == null || imageNameArray.length < 1) {
      logger.warn(
        "Code Block does not contain a diagram name [%% diagramName:<file_name>]. Block will be ignored."
      );
      return x;
    }
    ...
    if (imageNameArray.length != 2) {
      logger.warn(
        "Code Block does not contain a valid diagram name [%% diagramName:<file_name>]. Block will be ignored."
      );
      return x;
    }
    ...
    if (!skipDiagrams) {
      ...
      try {
        fs.writeFileSync(
          path.join(mermaidPath, _mermaidTempFile),
          contentArray.join("\n")
        );
      } catch (error) {
        logger.warn(error);
      ...
      shell.exec(
        __dirname +
          "/../node_modules/.bin/mmdc -w 2048 -H 1536 -c " +
  ...
  if (!skipDiagrams) {
    try {
      ...
      logger.info(
        "Successfully deleted " + path.join(mermaidPath, _mermaidTempFile)
      );
    } catch (err) {
      logger.warn(
        "Could not delete " +
