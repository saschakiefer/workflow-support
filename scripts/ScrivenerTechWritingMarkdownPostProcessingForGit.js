  ...
  const sections = [
    {
      ...
      content:
        "Executes some post processing scripts after Scrivener compilation (e.g. TOC generation, Footnote processing...)"
    },
    {
      header: "Options",
      optionList: [
...
function fixImageURL() {
  ...
  var res = str.replace(regex, function(x) {
    ...
    if (anchorArray.length != 1) {
      logger.error(
        "Found multiple matches for the Image Anchor [" +
          anchorName +
          "] -> Exiting"
      );
      process.exit(4);
    ...
    logger.info(
      "Extracting data from: " +
        anchorDefinition.substr(0, anchorDefinition.length - 1)
  ...
  for (var i = 0; i < fileAnchorArray.length; i++) {
    logger.info(
      "Removing: " + fileAnchorArray[i].substr(0, fileAnchorArray[i].length - 1)
    ); // Removing the trailing \n
